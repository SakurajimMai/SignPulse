...
| 变量 | 说明 |
| --- | --- |
| `APP_DATABASE_URL` | 可选 SQLAlchemy URL；设置后优先于本地 `db.sqlite`。MariaDB/MySQL 用 `mysql+pymysql://...` |
...
1. **数据目录**：把宿主机相对路径 `./data` 挂到容器 `APP_DATA_DIR`（`/data`），含 `db.sqlite` / `sessions` / `.signer`，确认可写。不要写成 `/data:/data`。
...
4. **云库（可选）**：`APP_DATABASE_URL` 可指向云 MariaDB（镜像已带 PyMySQL）或 Postgres（需自备 `psycopg2-binary`）。启动时会 `create_all` 建表；session 文件仍在 `./data`。
...
1. **停止** 面板服务（避免写冲突）  
2. 将备份解压到宿主机数据目录（Compose 为相对路径 `./data`，对应容器内 `APP_DATA_DIR=/data`），覆盖同名路径  
3. 确认目录属主/权限可写  
4. **启动** 服务，用原账号登录验证  
