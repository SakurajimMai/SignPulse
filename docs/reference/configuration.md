...
| `APP_DATA_DIR` | `/data` | | 容器内数据目录。Compose 宿主机侧用相对路径 `./data`，不要挂系统根目录 `/data` |
...
| `APP_DATABASE_URL` | （空=本地 SQLite） | | SignPulse 主数据库 URL。可指向云 MariaDB/MySQL（`mysql+pymysql://user:pass@host:3306/db?charset=utf8mb4`）或 Postgres；漫画库用独立的 `MANGA_DATABASE_URL` |
...
漫画是 SignPulse 内的可选子模块，建议使用 `MANGA_*` 前缀配置，旧项目的 `TELEGRAM_*`、`TG_SOURCE_CHATS` 等名称仍兼容。

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `MANGA_ENABLED` | `false` | 是否启用自动监听；未设置时，完整配置会自动启用 |
| `MANGA_DATA_DIR` | `APP_DATA_DIR` | 漫画临时目录、配置文件和默认数据库所在目录 |
| `MANGA_DATABASE_URL` | `sqlite:////data/tg_manga.db` | 漫画目录数据库，必须与面板库分开；可指向同一台云 MariaDB 上的另一个库 |
| `MANGA_TELEGRAM_ACCOUNT` | 空 | 采集使用的 SignPulse 账号名（`/accounts`） |
| `MANGA_HISTORY_BACKFILL_LIMIT` | `1500` | 启动时回填每个讨论群最近多少条消息；`0` 关闭 |
| `MANGA_TELEGRAM_API_ID` / `MANGA_TELEGRAM_API_HASH` | 空 | 可选覆盖；默认使用面板 Telegram API 凭证 |
| `MANGA_SOURCE_BINDINGS` | 空 | JSON：`[{"channel":"@chan","discussion":"@group","ingest_mode":"auto"}]`。`ingest_mode` 可为 `auto` / `discussion` / `telegraph`；讨论群留空且 auto 时走 Telegraph |
| `MANGA_SOURCE_CHATS` | 空 | 兼容旧配置；逗号分隔。有 `source_bindings` 时不再作为主配置 |
| `MANGA_ALLOWED_SENDER_IDS` | 空 | 可选；配置后只接收这些发送者 |
| `MANGA_DISCUSSION_ONLY` | `true` | 漫画图只采集讨论群回复；采集频道里的视频仍会转发到运营频道，不会进主站 |
| `MANGA_IGNORE_USER_COMMENTS` | `true` | 过滤普通用户评论 |
| `MANGA_CHAPTER_MAX_PAGES` | `200` | 单话最大页数（面板/API 上限 10000；长篇可设 2000） |
| `CFBED_UPLOAD_URL` | 空 | ImgBed 完整上传接口 |
| `SITE_PUBLISH_URL` | 空 | AnimeStream 的 `/api/manga/publish` 地址 |
| `SITE_PUBLISH_SECRET` | 空 | AnimeStream 后台生成的漫画发布密钥 |
| `MANGA_OUTBOUND_ENABLED` | `false` | 新章节入库后转发到自己的频道；先发漫画预览，视频可选跟在后面。主站仍只收漫画图 |
| `MANGA_OUTBOUND_CHANNEL` | 空 | 目标频道，如 `-1002085070183`；采集号需为管理员 |
| `MANGA_OUTBOUND_PREVIEW_COUNT` | `4` | 预览图数量，1–10 |
| `MANGA_OUTBOUND_BUTTON_TEXT` | `点击阅读` | 阅读链接文案 |
| `MANGA_OUTBOUND_SITE_BASE` | 空 | 阅读页站点，默认取 `SITE_PUBLISH_URL` 的域名 |
| `MANGA_OUTBOUND_BOT_TOKEN` | 空 | 可选。填写后用 Bot 发相册并挂「点击阅读」按钮；留空则用采集号发文字链接 |
| `MANGA_OUTBOUND_FORWARD_VIDEOS` | `true` | 是否转发源频道/讨论区剧情视频；关闭则只发漫画预览 |
| `MANGA_OUTBOUND_VIDEO_BLOCK_KEYWORDS` | 空 | 视频屏蔽词，逗号或换行分隔；标题/配文命中则不转发 |
...
| `MANGA_UPLOAD_TELEGRAM` | `imgbed` | 频道采集图片去向：`imgbed` / `ftp` / `sftp` / `s3` / `r2` / `b2` |
...
| `MANGA_S3_ENDPOINT` | 空 | S3 兼容 Endpoint。R2：`https://<accountid>.r2.cloudflarestorage.com`；B2：`https://s3.<region>.backblazeb2.com`；AWS 可留空 |
| `MANGA_S3_REGION` | 空 | R2 默认 `auto`；B2 默认 `us-west-004`；AWS 默认 `us-east-1` |
| `MANGA_S3_BUCKET` | 空 | Bucket 名 |
| `MANGA_S3_ACCESS_KEY` / `MANGA_S3_SECRET_KEY` | 空 | 对象存储密钥（落盘加密） |
| `MANGA_S3_PREFIX` | `manga` | 对象键前缀；文件为 `{prefix}/年/月/日/{uuid}/001.jpg` |
| `MANGA_S3_PUBLIC_BASE` | 空 | 对应前缀目录的公网基址 |
...
Compose 下宿主机是相对路径 `./data`，容器内是 `/data`（`APP_DATA_DIR`）。不要把 volumes 写成 `/data:/data`。
