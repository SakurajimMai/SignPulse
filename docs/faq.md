...
- 不设置 `APP_DATABASE_URL` → 使用 SQLite  
- 设置 `APP_DATABASE_URL=mysql+pymysql://user:pass@host:3306/db?charset=utf8mb4` → 云 MariaDB / MySQL  
- 设置 `APP_DATABASE_URL=postgresql+psycopg2://...` 并安装 `psycopg2-binary` → PostgreSQL  

项目**支持**云 MariaDB 与 Postgres，但**不强制**迁移，也未移除 SQLite 路径。Compose **不会**随应用启动数据库容器，把 URL 指到云厂商实例即可。

## 为什么重启后数据丢了

通常是因为没有把宿主机相对目录 `./data` 挂到容器内 `/data`，或者容器内 `/data` 不可写，程序降级到了 `/tmp/tg-signpulse`。不要写成 `/data:/data`（那会用系统根目录）。
