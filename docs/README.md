...
| 部署 | Docker Compose（推荐）、GHCR `ghcr.io/sakurajimmai/signpulse` |
...
| 文档 | 说明 |
|------|------|
| [Docker 部署](deploy/docker.md) | **推荐 Compose** 拉取 Docker Hub 镜像、升级与持久化 |
...
Compose 把宿主机相对路径 `./data` 挂到容器内 `/data`。不要写成 `/data:/data`（系统根目录）。
