...
async def _job_run_sign_task(
    ...
    try:
        ...
        if (
            task_config
            and task_config.get("execution_mode") == "range"
            and not skip_range_delay
        ):
            ...
            if range_start_str and range_end_str:
                try:
                    ...
                    try:
                        saved_tz = get_config_service().get_global_settings().get(
                            "timezone"
                        )
                        if saved_tz:
                            tz_name = str(saved_tz)
                    except Exception:
                        pass
                    now = localize_now(str(tz_name or "Asia/Hong_Kong"))
                    if last_run_is_today(task_config, now):
                        logger.info(
                            "Scheduler: 任务 %s 今日已执行，跳过", task_name
                        )
                        return
                    ...
                    if total_seconds > 0:
                        ...
                        run_at = compute_range_run_at(
                            now, task_config, delay_seconds
                        )
                        if (run_at - now).total_seconds() > 20:
                            job_id = oneshot_job_id(
                                account_name, task_name, now.date().isoformat()
                            )
                            ...
                            logger.info(
                                "Scheduler: 任务 %s 已改约到 %s，避免长 sleep 被重启冲掉",
                                task_name,
                                run_at.isoformat(),
                            )
                            return
        ...
        schedule_alert(
            ...
            title=f"签到调度任务异常退出：{account_name}/{task_name}",
            detail=str(e),
            fingerprint=f"sign-job:{account_name}:{task_name}",
        )
...
async def _job_device_keepalive() -> None:
    ...
    try:
        ...
        if int(result.get("failed") or 0) > 0:
            ...
            failures = [
                f"{item.get('account_name') or '-'}: {item.get('message') or '保活失败'}"
            ...
            if failures:
                schedule_alert(
                    ...
                    title=f"设备保活失败：{len(failures)} 个账号",
                    detail="\n".join(failures[:20]),
                    fingerprint="device-keepalive",
                )
    except Exception as exc:
    ...
    try:
        ...
        if not result.get("success"):
            ...
            schedule_alert(
                ...
                title="本地自动备份失败",
                detail=f"{fail_reason}\npath={result.get('path') or '-'}",
                fingerprint="auto-backup:archive",
            )
            ...
            schedule_alert(
                ...
                title="WebDAV 备份上传失败",
                detail=f"{fail_reason}\npath={result.get('path') or '-'}",
                fingerprint="auto-backup:webdav",
            )
        remote_prune = result.get("remote_prune") or {}
        if isinstance(remote_prune, dict) and remote_prune.get("success") is False:
            from backend.services.alerts import schedule_alert

            schedule_alert(
                "system_backup_retention_fail",
                title="WebDAV 旧备份清理失败",
                detail=str(remote_prune.get("error") or "远端保留策略执行失败"),
                fingerprint="auto-backup:retention",
            )
    except Exception as exc:
...
def _sync_auto_backup_job() -> None:
    ...
    try:
        ...
        schedule_alert(
            ...
            title="自动备份调度注册失败",
            detail=str(exc),
            fingerprint="scheduler-register:auto-backup",
        )
    except Exception as exc:
...
def _sync_games_cloud_keepalive_job() -> None:
    """定时访问网盘接口并写回 Cookie / Token，避免登录态过期。"""
    global scheduler
    if scheduler is None:
        return
    ...
    try:
        ...
        scheduler.add_job(
            refresh_cloud_sessions,
            trigger=IntervalTrigger(hours=KEEP_INTERVAL_HOURS),
            id=KEEPALIVE_JOB_ID,
            replace_existing=True,
            max_instances=1,
            coalesce=True,
        )
        logger.info("游戏网盘续期任务已注册：每 %s 小时", KEEP_INTERVAL_HOURS)
    except (ImportError, AttributeError, ValueError, KeyError, RuntimeError) as exc:
        logger.warning("同步游戏网盘续期任务失败: %s", exc)
        try:
            scheduler.remove_job("games-cloud-keepalive")
        except JobLookupError:
            pass
        from backend.services.alerts import schedule_alert
        ...
        schedule_alert(
            ...
            title="网盘续期调度注册失败",
            detail=str(exc),
            fingerprint="scheduler-register:cloud-keepalive",
        )
    except Exception as exc:
        logger.exception("同步游戏网盘续期任务发生未知异常")
        from backend.services.alerts import schedule_alert
...
async def sync_jobs() -> None:
    ...
    try:
        ...
        _sync_games_cloud_keepalive_job()
...
async def catch_up_missed_sign_tasks() -> None:
    """容器启动时：今日未签且仍在时间窗内，立刻补跑，不再空等明天。"""
    global scheduler
    if scheduler is None:
        return
    from backend.core.config import get_settings
    ...
    tz_name = get_settings().timezone
    try:
        saved_tz = get_config_service().get_global_settings().get("timezone")
        if saved_tz:
            tz_name = str(saved_tz)
    except Exception:
        pass
    ...
    for task in service.list_tasks(force_refresh=False):
        if not should_catch_up_sign_task(task, now):
            continue
        account_name = str(task.get("account_name") or "").strip()
        task_name = str(task.get("name") or "").strip()
        if not account_name or not task_name:
            continue
        job_id = oneshot_job_id(account_name, task_name, now.date().isoformat())
        ...
        logger.info(
            "Scheduler: 补跑今日未执行的签到 %s/%s，将于 %s 启动",
            account_name,
            task_name,
            run_at.isoformat(),
        )
