...
from backend.core.db_url import is_mysql_url, is_sqlite_url, normalize_sync_url
...
class Settings(BaseModel):
    ...
    # 可选完整 SQLAlchemy URL；设置后优先于 sqlite 文件路径（Postgres / 云 MariaDB 等）
    ...
    def from_environment(cls) -> "Settings":
        ...
        return cls(
            ...
            # DATABASE_URL is reserved by the legacy manga submodule. The
            # SignPulse database must be explicit to prevent accidental reuse.
            database_url_override=_read_env(env, "APP_DATABASE_URL", default=None),
    ...
    def database_url(self) -> str:
        if self.database_url_override:
            return normalize_sync_url(str(self.database_url_override).strip())
    ...
    def is_mysql(self) -> bool:
        return is_mysql_url(self.database_url)
