...
def _reveal_secret(value: object, *, reveal: bool) -> Optional[str]:
    """Return a persisted secret as plaintext only for an explicit reveal."""
    if not reveal or value is None:
        return None
    text = str(value)
    if not text:
        return None
...
def get_ai_config(
    ...
    try:
        ...
        return AIConfigResponse(
            ...
            api_key=str(api_key) if reveal and api_key else None,
...
class GlobalSettingsRequest(BaseModel):
    ...
    # Deprecated write-only URL accepted for clients from before structured
    # proxy settings. New clients should use the proxy_* fields below.
    ...
    proxy_clear_credentials: Optional[bool] = None
    ...
    telegram_bot_control_enabled: Optional[bool] = None
    telegram_bot_mini_app_url: Optional[str] = None
    telegram_bot_allowed_user_ids: Optional[list[int]] = None
...
class GlobalSettingsResponse(BaseModel):
    ...
    data_dir: Optional[str] = None
    # Internal compatibility URL is always removed from API responses.
    global_proxy: Optional[str] = None
    ...
    telegram_bot_chat_id: Optional[str] = None
    telegram_bot_message_thread_id: Optional[int] = None
    telegram_bot_control_enabled: bool = False
    telegram_bot_mini_app_url: Optional[str] = None
    telegram_bot_allowed_user_ids: list[int] = Field(default_factory=list)
    ...
    webdav_url: Optional[str] = None
    webdav_username: Optional[str] = None
    # GET 默认不回传密码；用 webdav_password_set 表示已落盘
    webdav_password: Optional[str] = None
...
class TelegramBotRuntimeIdentity(BaseModel):
    id: Optional[int] = None
    username: str = ""
    first_name: str = ""


class TelegramBotRuntimeStatusResponse(BaseModel):
    state: str = "stopped"
    configured: bool = False
    control_enabled: bool = False
    primary: bool = False
    running: bool = False
    webhook_conflict: bool = False
    polling_conflict: bool = False
    bot: TelegramBotRuntimeIdentity = Field(
        default_factory=TelegramBotRuntimeIdentity
    )
    last_error: str = ""
...
def get_global_settings(
    ...
    try:
        ...
        # 默认脱敏；reveal=1 时管理员点小眼睛才回说明文。
        settings["global_proxy"] = None
        raw_proxy_password = settings.get("proxy_password")
        settings["proxy_password_set"] = bool(
            raw_proxy_password is not None
            and str(raw_proxy_password).strip() != ""
        )
        settings["proxy_password"] = _reveal_secret(
            raw_proxy_password, reveal=reveal
        )
        raw_pwd = settings.get("webdav_password")
        settings["webdav_password_set"] = bool(
            raw_pwd is not None and str(raw_pwd).strip() != ""
        )
        settings["webdav_password"] = _reveal_secret(raw_pwd, reveal=reveal)
        ...
        settings["telegram_bot_token"] = _reveal_secret(raw_token, reveal=reveal)
        raw_smtp = settings.get("smtp_password")
        settings["smtp_password_set"] = bool(
            raw_smtp is not None and str(raw_smtp).strip() != ""
        )
        settings["smtp_password"] = _reveal_secret(raw_smtp, reveal=reveal)
...
async def test_global_proxy(current_user: User = Depends(get_current_user)):
    """Test the saved proxy only against Telegram's fixed public endpoint."""

    service = get_config_service()
    try:
        proxy_url = service.get_global_proxy()
    except RuntimeError:
    ...
    from backend.services.proxy_settings import check_proxy_connection

    result = await check_proxy_connection(proxy_url)
    return ProxyTestResponse(**result)


@router.get(
    "/settings/telegram-bot/runtime",
    response_model=TelegramBotRuntimeStatusResponse,
)
def get_telegram_bot_runtime_status(
    current_user: User = Depends(get_current_user),
):
    """Return a token-free runtime snapshot for the administrator panel."""
    from backend.services.telegram_bot import get_telegram_bot_runtime

    snapshot = get_telegram_bot_runtime().status()
    snapshot["polling_conflict"] = snapshot.get("state") == "polling_conflict"
    return TelegramBotRuntimeStatusResponse(**snapshot)
...
async def save_global_settings(
    ...
    try:
        ...
        proxy_runtime_fields = {
            ...
            "proxy_password",
            "proxy_no_proxy",
            "proxy_clear_credentials",
        }
        if proxy_runtime_fields.intersection(settings):
            from backend.services.proxy_runtime import (
                apply_proxy_environment,
                schedule_proxy_runtime_reconcile,
            )

            # Environment-backed short-lived clients use the new proxy before
            # this response returns. Persistent workers rotate in the background.
            apply_proxy_environment()
            schedule_proxy_runtime_reconcile()
        bot_runtime_fields = {
            "telegram_bot_token",
            "telegram_bot_control_enabled",
            "telegram_bot_mini_app_url",
            "telegram_bot_allowed_user_ids",
        }
        if bot_runtime_fields.intersection(settings):
            from backend.services.telegram_bot import get_telegram_bot_runtime
...
async def test_bot_notification(
    ...
    if not bot_token or not bot_notification_targets(cfg):
        return BotTestResponse(success=False, message="未配置 Bot Token 或用户 ID")
    raw_msg = (request.message or "").strip()
    text = raw_msg or "TG-SignPulse 通知测试：连接正常"
    # 限制长度，避免误填超大文本导致 Telegram API 失败
    text = text[:3900]
    try:
        await send_telegram_notifications(cfg, text=text)
