...
class MemoryMonitor:
    ...
    参数:
        ...
        alert_repeat_interval: 持续超限时再次调用告警回调的间隔；默认不重报
    """
    ...
    alert_repeat_interval: Optional[float] = None
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.alert_repeat_interval is not None
            and self.alert_repeat_interval <= 0
        ):
            raise ValueError("alert_repeat_interval 必须 > 0")
    ...
    _last_alert_monotonic: Optional[float] = field(
        default=None, init=False, repr=False
    )
    ...
    def check(self) -> Optional[AlertRecord]:
        ...
        默认在连续超阈值期间只告警首次；配置 alert_repeat_interval 后按该
        间隔重报，使上层投递失败或暂时被抑制时仍有后续触发机会；
        GC 仍按轮执行（幂等，不因去重而跳过）。
        """
        snap = self.snapshot()
        if snap.rss_mb >= self.threshold_mb:
            ...
            should_alert = not self._in_alert
            if (
                not should_alert
                and self.alert_repeat_interval is not None
                and self._last_alert_monotonic is not None
                and monotonic_now - self._last_alert_monotonic
                >= self.alert_repeat_interval
            ):
                should_alert = True
            if should_alert:
                self._in_alert = True
                self._last_alert_monotonic = monotonic_now
