...
def bot_notification_targets(
    settings: Dict[str, Any],
) -> list[tuple[str, Optional[int]]]:
    """通知发到配置的用户 ID；若仍填写了群 Chat ID 则额外发一份。"""
...
def _bot_config(settings: Dict[str, Any]) -> tuple[str, str, Optional[int]]:
    ...
    """
    raw_token = str(settings.get("telegram_bot_token") or "").strip()
    token = raw_token
    if raw_token:
        from tg_signer.security import decrypt_secret, is_encrypted_secret

        if is_encrypted_secret(raw_token):
            try:
                token = str(decrypt_secret(raw_token) or "").strip()
            except Exception:
...
def telegram_bot_ready(settings: Dict[str, Any]) -> bool:
    """Return whether the global Telegram alert transport is usable."""
    token, _chat_id, _thread_id = _bot_config(settings)
    return bool(
        settings.get("telegram_bot_notify_enabled")
        and token
        and bot_notification_targets(settings)
    )
...
async def send_telegram_notifications(
    ...
    *,
    ...
    """把同一条通知发到全部用户 ID（以及可选的遗留 Chat ID）。"""
    token, _chat_id, _thread_id = _bot_config(settings)
    targets = bot_notification_targets(settings)
    if not token or not targets:
        raise RuntimeError("未配置 Bot Token 或用户 ID")
    last_error: Exception | None = None
    failures = 0
    for target_index, (chat_id, thread_id) in enumerate(targets, start=1):
        try:
    ...
    if failures:
        raise RuntimeError(
            f"Telegram 通知有 {failures}/{len(targets)} 个目标发送失败"
        ) from last_error


def send_telegram_notifications_sync(
    settings: Dict[str, Any],
    *,
    text: str,
    parse_mode: Optional[str] = None,
...
async def send_telegram_bot_message(
    *,
    ...
    last_error = "Telegram Bot API request failed"
    for attempt in (1, 2):
        try:
            async with httpx.AsyncClient(
                **httpx_async_client_kwargs(timeout=10)
            ) as client:
                response = await client.post(url, json=payload)
                response.raise_for_status()
                _validate_telegram_result(response, bot_token=bot_token)
            return
        except _TelegramResponseError as exc:
            last_error = str(exc)
            if not exc.retryable:
                raise RuntimeError(last_error) from None
            if attempt == 1:
                logger.warning("Telegram 通知响应无效，准备重试: %s", last_error)
                await asyncio.sleep(1.0)
        except (httpx.RequestError, httpx.HTTPStatusError) as exc:
            last_error = _telegram_error_summary(exc, bot_token=bot_token)
            if isinstance(exc, httpx.HTTPStatusError) and exc.response.status_code < 500:
                exc.args = (last_error,)
                raise exc from None
            if attempt == 1:
                logger.warning("Telegram 通知发送失败，准备重试: %s", last_error)
                await asyncio.sleep(1.0)
...
def _validate_telegram_result(response: Any, *, bot_token: str) -> None:
    """Reject proxy/edge HTML and Telegram ``ok=false`` 2xx responses."""
    try:
    ...
    if isinstance(payload, dict) and payload.get("ok") is True:
        return
    description = "Telegram Bot API rejected the message"
    error_code = 0
    retryable = True
    if isinstance(payload, dict):
        ...
        try:
            ...
            error_code = 0
        retryable = error_code == 429 or error_code >= 500 or error_code <= 0
    suffix = f" ({error_code})" if error_code else ""
    raise _TelegramResponseError(
        f"Telegram Bot API rejected the message{suffix}: {description[:240]}",
        retryable=retryable,
    )


def _telegram_error_summary(exc: Exception, *, bot_token: str = "") -> str:
    if isinstance(exc, httpx.HTTPStatusError):
        ...
        return f"Telegram Bot API HTTP {status_code}{suffix}"
    return f"Telegram Bot API network error ({type(exc).__name__})"
...
def send_telegram_bot_message_sync(
    *,
    ...
    """Synchronous transport used by the thread-based alert dispatcher."""
    payload = _telegram_message_payload(
        chat_id=chat_id,
        text=text,
        message_thread_id=message_thread_id,
    ...
    for attempt in (1, 2):
        try:
            with httpx.Client(**httpx_client_kwargs(timeout=10)) as client:
            ...
            if not exc.retryable:
                raise RuntimeError(last_error) from None
            if attempt == 1:
                logger.warning("Telegram 告警响应无效，准备重试: %s", last_error)
                time.sleep(1.0)
        except (httpx.RequestError, httpx.HTTPStatusError) as exc:
            last_error = _telegram_error_summary(exc, bot_token=bot_token)
            if isinstance(exc, httpx.HTTPStatusError) and exc.response.status_code < 500:
                raise RuntimeError(last_error) from None
            if attempt == 1:
                logger.warning("Telegram 告警发送失败，准备重试: %s", last_error)
                time.sleep(1.0)
    raise RuntimeError(last_error)
...
async def send_keyword_push(settings: Dict[str, Any], payload: Dict[str, Any]) -> None:
    ...
    if channel == "telegram":
        bot_token, _chat_id, _thread_id = _bot_config(settings)
        if not bot_token or not bot_notification_targets(settings):
    ...
    async with httpx.AsyncClient(**httpx_async_client_kwargs(timeout=10)) as client:
...
async def send_auto_backup_failure_notification(
    ...
    *,
    ...
    仅依赖通知总开关 + 已配置 Token/用户 ID；不绑定任务失败开关
