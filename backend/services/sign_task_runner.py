...
def _alert_fingerprint(account_name: str, task_name: str) -> str:
    """Encode the compound identity without delimiter collisions."""
    return json.dumps(
        [str(account_name), str(task_name)],
        ensure_ascii=False,
        separators=(",", ":"),
    )
...
async def _runner_send_notifications(state: Dict[str, Any]) -> None:
    ...
    if (
        ...
        detail_parts = [state.get("error_msg", "") or state.get("last_reply", "")]
        if failure_category:
            detail_parts.append(f"failure_category={failure_category}")
        alert_rule = {
            FailureCategory.FLOOD_WAIT.value: "sign_task_rate_limited",
            FailureCategory.AI_TIMEOUT.value: "sign_task_ai_fail",
            FailureCategory.AI_ERROR.value: "sign_task_ai_fail",
            FailureCategory.NETWORK_PROXY.value: "sign_task_network_fail",
            FailureCategory.TIMEOUT.value: "sign_task_timeout",
        }.get(failure_category, "sign_task_flow_fail")
        schedule_alert(
            alert_rule,
            title=f"签到任务失败：{state['account_name']}/{state['task_name']}",
            detail="\n".join(str(item) for item in detail_parts if item),
            fingerprint=_alert_fingerprint(
                state["account_name"], state["task_name"]
            ),
            channel_overrides={
                "telegram": bool(state.get("task_notify_on_failure", True))
            },
        )
