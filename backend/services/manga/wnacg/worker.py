...
class WnacgWorker:
    def __init__(self, settings: MangaSettings, client_provider: Any | None = None):
        ...
        self.store = MediaStore(settings, "wnacg")
    ...
    async def _loop(self) -> None:
        ...
        if not upload_configured(settings, "wnacg"):
            raise WnacgError(upload_missing_message(settings, "wnacg"))
        ...
        for image_url in image_urls:
            ...
            try:
                uploaded = await self.store.upload_bytes(
                    data,
                    filename=f"wnacg{ref.aid}-{stored:03d}.jpg",
    ...
    async def _outbound(
        ...
        if not publisher_enabled_for(self.channel, "wnacg"):
            return
