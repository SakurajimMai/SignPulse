...
class SignTaskService(SignTaskHistoryMixin, SignTaskCrudMixin):
    ...
    def _get_effective_proxy(self, account_name: str) -> Optional[str]:
        account_proxy = resolve_effective_proxy(account_name)
        if account_proxy:
            return account_proxy

        from backend.services.config import get_config_service

        # A broken enabled global proxy must stop the task instead of silently
        # constructing a direct Pyrogram connection.
