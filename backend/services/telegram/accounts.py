...
def _account_proxy_dict(account_name: str) -> Optional[dict]:
    ...
    if not proxy_value:
        return None
    proxy = build_proxy_dict(proxy_value)
    if proxy is None:
        raise ValueError(f"账号 {account_name} 的代理配置无效")
    return proxy
...
class TelegramAccountsMixin:
    ...
    async def download_account_avatar(self, account_name: str) -> Optional[bytes]:
        ...
        proxy_dict = _account_proxy_dict(account_name)
