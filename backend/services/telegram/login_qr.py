...
class TelegramQrLoginMixin:
    ...
    async def _expire_qr_login(self, login_id: str, expires_ts: int) -> None:
        while True:
            ...
            if data.get("status") == "success":
                ...
                return
            # 已扫码 / 等 2FA 时不能按 30 秒 token 拆掉会话，否则账号来不及落盘
            if (
                data.get("status") in {"password_required", "scanned_wait_confirm"}
                or data.get("scan_seen")
            ):
                self._extend_qr_expires(data)
                expires_ts = int(data.get("expires_ts") or expires_ts)
                continue
    ...
    async def _export_login_token(self, client, data: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            logger.debug("ExportLoginToken 失败", exc_info=True)
            return None
    ...
    async def get_qr_login_status(self, login_id: str) -> Dict[str, Any]:
        ...
        # 扫码后先续期，绝不能按原 token 的 30 秒把确认中的会话清掉
        if data.get("status") == "scanned_wait_confirm" or data.get("scan_seen"):
            data["scan_seen"] = True
            data["status"] = "scanned_wait_confirm"
            self._extend_qr_expires(data)
        elif time.time() >= data.get("expires_ts", 0):
            self._log_qr_state(login_id, "expired", data)
            await self._cleanup_qr_login(login_id)
            return {
                "status": "expired",
        ...
        client = data.get("client")
        if client is None:
            self._log_qr_state(login_id, data.get("status", "waiting_scan"), data)
            return {
                "status": data.get("status", "waiting_scan"),
                "expires_at": data.get("expires_at"),
                "message": "已扫码，请在手机上确认登录"
        ...
        try:
            ...
            # 官方确认流程：收到 UpdateLoginToken 后再次 ExportLoginToken
            if data.get("status") == "scanned_wait_confirm":
                ...
                if isinstance(outcome, raw.types.auth.LoginToken):
                    self._apply_login_token_update(data, outcome)
                    data["status"] = "scanned_wait_confirm"
                    logger.info(
                        "QR 已扫码待手机确认 login_id=%s", login_id
                    )
