...
from backend.utils.outbound import httpx_client_kwargs
...
def upload_file_to_webdav(
    *,
    ...
    with httpx.Client(
        **httpx_client_kwargs(
            timeout=req_timeout, auth=auth, follow_redirects=True
        )
    ) as client:
...
def list_webdav_files(
    *,
    ...
    with httpx.Client(
        **httpx_client_kwargs(timeout=timeout, auth=auth, follow_redirects=True)
    ) as client:
        resp = client.request(
            "PROPFIND",
            target,
