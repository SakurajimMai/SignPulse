...
| 数据库 | 默认 SQLite (WAL)；可选 `APP_DATABASE_URL` 指向云 MariaDB/MySQL 或 PostgreSQL |
...
| `APP_DATA_DIR` | 自动检测 | 容器内数据目录；Compose 宿主机侧用 `./data`，不要挂系统根目录 `/data` |
| `APP_SECRET_KEY` | 自动生成 | JWT 签名密钥 |
| `APP_ACCESS_TOKEN_EXPIRE_HOURS` | `12` | Token 有效期 |
| `APP_DATABASE_URL` | 空 | 非空时覆盖 SQLite；MariaDB/MySQL 例：`mysql+pymysql://user:pass@host:3306/db?charset=utf8mb4` |
...
| 编排 | `docker-compose.panel.yml`：宿主机相对路径 `./data` 挂到容器 `/data`，`APP_SCHEDULER_LOCK=1`，探针 `/readyz` |
