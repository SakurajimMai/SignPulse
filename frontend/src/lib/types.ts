...
const API_ERROR_CODE_MESSAGES: Record<string, string> = {
  ...
  MINI_APP_DISABLED: 'Mini App control is disabled in system settings',
  MINI_APP_OPERATOR_NOT_ALLOWED: 'This Telegram user is not authorized to use the Mini App',
  MINI_APP_INIT_DATA_INVALID: 'Telegram authorization is invalid or expired; reopen the Mini App from the Bot',
  MINI_APP_AUTH_RATE_LIMITED: 'Too many Mini App authorization attempts; please retry later',
  MINI_APP_NOT_AUTHENTICATED: 'Mini App is not authenticated; reopen it from Telegram',
  MINI_APP_SESSION_INVALID: 'Mini App session is invalid or expired; reopen it from the Bot',
  MINI_APP_SESSION_REVOKED: 'Mini App session was revoked; reopen it from the Bot',
  MINI_APP_RUN_RATE_LIMITED: 'Too many task operations; please retry later',
  MINI_APP_PRIMARY_UNAVAILABLE: 'The primary service is temporarily unavailable; please retry shortly',
  ORIGIN_HTML_ERROR: 'The origin returned a web error page (often Cloudflare 520/522); retry later',
}
...
const HTML_DOCUMENT_MARKER = /<!doctype\s+html\b|<html(?:\s|>)|<!--\[if/i
const HTML_TITLE_RE = /<title[^>]*>([^<]+)/i
const CF_STATUS_RE = /\b(52[0-7])\b/

export function looksLikeHtmlError(text: string): boolean {
  // JSON.parse 的错误通常带有 `Unexpected token ... "<!DOCTYPE ..."` 前缀，
  // 因此不能只检查字符串开头；限制在前 1 KiB 可避免扫描超长日志正文。
  return HTML_DOCUMENT_MARKER.test((text || '').slice(0, 1024))
}

function htmlErrorFallback(text: string, status?: number): string {
  const title = text.match(HTML_TITLE_RE)?.[1]?.replace(/\s+/g, ' ').trim() || ''
  const phrase = title.includes('|') ? title.split('|').pop()!.trim() : title
  const code =
    phrase.match(CF_STATUS_RE)?.[1] ||
    (status && status >= 520 && status <= 527 ? String(status) : '')
  if (code) return `Request failed (${code}): origin HTML error`
  if (status) return `Request failed (${status}): origin HTML error`
  return 'Request failed: origin HTML error'
}

function truncateErrorMessage(text: string): string {
  if (looksLikeHtmlError(text)) return htmlErrorFallback(text)
