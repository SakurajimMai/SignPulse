...
import { looksLikeHtmlError, type ApiError, type FastApiValidationError } from "../types";
...
const ORIGIN_HTML_ERROR = "ORIGIN_HTML_ERROR";

function declaresHtmlResponse(res: Response): boolean {
...
function originHtmlError(status: number): ApiError {
  const err = new Error(ORIGIN_HTML_ERROR) as ApiError;
  err.status = status;
  err.code = ORIGIN_HTML_ERROR;
  return err;
}

function assertNotHtmlResponse(res: Response, body?: string): void {
  if (declaresHtmlResponse(res) || (body != null && looksLikeHtmlError(body))) {
    throw originHtmlError(res.status);
  }
}
...
export async function fetchWithAuth(
  ...
  if (!res.ok) {
    ...
    if (
      declaresHtmlResponse(res) ||
      looksLikeHtmlError(responseText) ||
      looksLikeHtmlError(errorMessage)
    ) {
      errorMessage = ORIGIN_HTML_ERROR;
      errorCode = ORIGIN_HTML_ERROR;
    }
  ...
  // 反向代理/API 路由异常可能以 2xx 返回 HTML。先按响应类型拦截，
  // 避免调用方的 json() 抛出包含整段 <!DOCTYPE> 的 SyntaxError。
  assertNotHtmlResponse(res);
  return res;
}
...
export async function request<T>(
  ...
  return requestWithTotalTimeout(
    ...
    async (res) => {
      ...
      try {
        // A valid JSON string may legitimately contain an upstream HTML error
        // captured in an alert detail. Parse first so that embedded diagnostics
        // are not mistaken for an HTML document returned by the origin.
        return JSON.parse(body) as T;
      } catch (error) {
        assertNotHtmlResponse(res, body);
        throw error;
      }
    },
...
export async function requestBlob(
  ...
  return requestWithTotalTimeout(
    ...
    async (res) => {
      ...
      assertNotHtmlResponse(res, prefix);
      return blob;
    },
  );
}
