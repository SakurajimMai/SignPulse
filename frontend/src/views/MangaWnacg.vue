...
import WnacgPanel from '../components/manga/WnacgPanel.vue'
...
const applySettings = (value: MangaSettings) => {
...
const saveSettings = async () => {
  ...
  try {
    const payload: Record<string, unknown> = {
      wnacg_enabled: Boolean(settings.value.wnacg_enabled),
      wnacg_base_url: settings.value.wnacg_base_url || 'https://www.wnacg.com',
      wnacg_categories: settings.value.wnacg_categories || '',
      wnacg_max_pages: Number(settings.value.wnacg_max_pages || 400),
      wnacg_list_pages: Number(settings.value.wnacg_list_pages || 1),
...
<template>
  <div class="mx-auto w-full space-y-6 pb-10">
    ...
    <template v-else>
      <div>
        <div class="ui-section-label mb-2">{{ t('manga.eyebrow') }} / {{ t('nav.mangaWnacg') }}</div>
        <h2 class="text-2xl font-medium tracking-tight text-gray-900 dark:text-gray-100">{{ t('manga.wnacgTitle') }}</h2>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-2 max-w-2xl">{{ t('manga.wnacgPageHint') }}</p>
      </div>
