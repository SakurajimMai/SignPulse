...
describe('useSettingsSave', () => {
  ...
  function setup(over?: {
    ...
    const afterProxySettingsSaved = vi.fn()
    const afterBotSettingsSaved = vi.fn(async () => {})
    ...
    const save = useSettingsSave({
      ...
      buildProxyPayload: () => ({ proxy_enabled: true }),
  ...
  it('saveProxySettings marks proxy clean and clears local secret state', async () => {
    api.saveGlobalSettings.mockResolvedValue({})
    const { save, afterProxySettingsSaved, markSectionClean } = setup()
    await save.saveProxySettings()
    expect(api.saveGlobalSettings).toHaveBeenCalledWith('tok', {
      proxy_enabled: true,
    })
    expect(afterProxySettingsSaved).toHaveBeenCalled()
    expect(markSectionClean).toHaveBeenCalledWith('proxy')
    expect(save.proxyLoading.value).toBe(false)
  })

  it('testProxy reports API result and resets loading', async () => {
    api.testProxyConnection.mockResolvedValue({ success: true, message: 'proxy ok' })
    const { save } = setup()
    await save.testProxy()
    expect(api.testProxyConnection).toHaveBeenCalledWith('tok')
    expect(toastSpy.success).toHaveBeenCalledWith('proxy ok')
    expect(save.proxyTestLoading.value).toBe(false)
  })

  it('saveBotSettings runs afterBotTokenSaved', async () => {
    api.saveGlobalSettings.mockResolvedValue({})
    const { save, afterBotTokenSaved, afterBotSettingsSaved, markSectionClean } = setup()
    await save.saveBotSettings()
    expect(afterBotTokenSaved).toHaveBeenCalled()
    expect(afterBotSettingsSaved).toHaveBeenCalledWith('tok')
  ...
  it('saveSmtpSettings marks smtp clean', async () => {
    api.saveGlobalSettings.mockResolvedValue({})
    const afterSmtpPasswordSaved = vi.fn()
    const markSectionClean = vi.fn()
    const save = useSettingsSave({
      ...
      buildAiRuntimePayload: () => ({}),
      buildBackupPayload: () => ({}),
      buildSmtpPayload: () => ({ smtp_enabled: true }),
      markSectionClean,
      afterBotTokenSaved: vi.fn(),
    ...
    await save.saveSmtpSettings()
    expect(api.saveGlobalSettings).toHaveBeenCalledWith('tok', { smtp_enabled: true })
    expect(afterSmtpPasswordSaved).toHaveBeenCalled()
    expect(markSectionClean).toHaveBeenCalledWith('smtp')
  })
