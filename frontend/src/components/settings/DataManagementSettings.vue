...
const emit = defineEmits<{
  ...
  (e: 'toggle-reveal', key: 'webdavPassword'): void
}>()
    ...
    <div class="pt-5 border-t border-gray-200 dark:border-gray-800/60 space-y-3">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
        ...
        <div class="space-y-1.5">
          ...
          <SecretInput
            :model-value="modelValue.webdavPassword"
            :revealed="Boolean(reveal?.webdavPassword)"
            :placeholder="webdavPasswordSet ? t('settings.webdavPasswordSavedHint') : t('settings.webdavPasswordHint')"
            autocomplete="current-password"
            @update:model-value="update('webdavPassword', $event)"
            @toggle="emit('toggle-reveal', 'webdavPassword')"
          />
        </div>
      </div>
      <div class="space-y-1.5">
