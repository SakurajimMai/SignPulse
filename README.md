...
| **漫画采集** | Telegram 群组过滤、E-Hentai / WNACG 站点源、作者/标签解析、首图封面、ImgBed 上传、AnimeStream 发布 |
...
| `APP_DATA_DIR` | 容器内数据目录（宿主机请挂 `./data`，不要用系统根目录 `/data`） | `/data` |
| `APP_DATABASE_URL` | 可选；空=SQLite，可设云 MariaDB/MySQL 或 Postgres URL | 空 |
...
SignPulse 是主应用，漫画采集只是其中一个可选模块，可在面板侧栏的「漫画」页面配置和启停。采集号直接使用 `/accounts` 里已登录的账号；漫画目录数据库独立，不会把漫画数据写进主面板库。

采集器按「采集频道 → 讨论群」一一对应监听。优先配置面板里的绑定列表，或环境变量 `MANGA_SOURCE_BINDINGS`；旧的 `MANGA_SOURCE_CHATS` / `TG_SOURCE_CHATS` 仍可作为回退。默认只采集讨论群回复，过滤普通用户评论、压缩包、视频、音频和下载推广。形如 `[Ryx] 标题 #标签` 的标题会保存为「标题」，作者为 `Ryx`，标签单独保存，第一张已上传图片作为封面；下载完成后临时文件会立即删除。同一讨论帖后续分批相册会追加到已有章节，不会在约 30 页处截断。

站点源：侧栏「漫画采集」下的 E-Hentai / WNACG 为独立二级栏目。WNACG 可指定绅士漫画分类（默认同人誌漢化），按间隔拉新本，图片仍走共用图床和主站发布。

请先在 `/accounts` 添加并登录采集账号，再到漫画页选择该账号。

主站发布密钥在 AnimeStream 后台生成后，填入漫画模块的 `SITE_PUBLISH_SECRET`，发布地址填入 `SITE_PUBLISH_URL`（例如 `https://你的主站/api/manga/publish`）。
