...
class TestSendAccountInvalidNotification:
    ...
    @pytest.mark.asyncio()
    async def test_user_id_without_chat_id_sends(self, notify_env):
        notify_env.settings["telegram_bot_chat_id"] = ""
        notify_env.settings["telegram_bot_allowed_user_ids"] = [42]
        await sign_task_notify.send_account_invalid_notification(
            account_name="a", task_name="t", message="失效"
        )
        assert len(notify_env.sent) == 1
        assert notify_env.sent[0]["chat_id"] == "42"
        assert notify_env.sent[0].get("message_thread_id") is None
...
def mark_env(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda rule_id, **kwargs: alerts.append({"rule_id": rule_id, **kwargs}),
    )
    monkeypatch.setattr(
...
class TestMarkAccountInvalid:
    ...
    async def test_first_invalid_marks_and_notifies(self, mark_env):
        ...
        assert mark_env.alerts == [
            {
                "rule_id": "account_login_invalid",
                "title": "Telegram 账号登录失效：acc1",
                "detail": "来源：签到任务 daily\nsession 失效",
                "fingerprint": "acc1",
    ...
    async def test_notify_disabled_still_returns_first_flag(self, mark_env):
        ...
        assert len(mark_env.set_calls) == 1
        assert mark_env.notified == []
        assert mark_env.alerts[0]["channel_overrides"] == {"telegram": False}
