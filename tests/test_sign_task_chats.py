...
def test_read_env_telegram_api_aliases(monkeypatch):
    ...
    monkeypatch.delenv("MANGA_TELEGRAM_API_ID", raising=False)
    monkeypatch.delenv("MANGA_TELEGRAM_API_HASH", raising=False)
    assert read_env_telegram_api() == (None, None)

    monkeypatch.setenv("TELEGRAM_API_ID", "20453784")
    monkeypatch.setenv("TELEGRAM_API_HASH", "hash-from-telegram")
    assert read_env_telegram_api() == ("20453784", "hash-from-telegram")

    monkeypatch.setenv("TG_API_ID", "111")
    monkeypatch.setenv("TG_API_HASH", "hash-from-tg")
    assert read_env_telegram_api() == ("111", "hash-from-tg")
