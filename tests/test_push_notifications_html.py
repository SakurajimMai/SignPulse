...
from backend.services.push_notifications import (
    ...
    send_telegram_notifications_sync,
)
...
class TestParseModePropagation:
    ...
    @pytest.mark.asyncio()
    async def test_success_status_html_response_is_retried_and_rejected(
        self, monkeypatch
    ):
        calls = {"n": 0}
        ...
        with pytest.raises(RuntimeError, match="invalid response"):
            await send_telegram_bot_message(
                bot_token="tok", chat_id="chat", text="hi"
            )
        assert calls["n"] == 2


class TestTelegramNotificationTargets:
    @staticmethod
    def _settings():
        return {
            "telegram_bot_token": "tok",
            "telegram_bot_allowed_user_ids": [1001, 1002],
        }

    @pytest.mark.asyncio()
    async def test_async_partial_target_failure_is_reported(self, monkeypatch):
        attempted: list[str] = []

        async def _send(**kwargs):
            attempted.append(kwargs["chat_id"])
        ...
        with pytest.raises(RuntimeError, match="1/2") as raised:
            await send_telegram_notifications(self._settings(), text="alert")

        assert attempted == ["1001", "1002"]
        assert "1001" not in str(raised.value)
        assert "1002" not in str(raised.value)

    def test_sync_partial_target_failure_is_reported(self, monkeypatch):
        attempted: list[str] = []
        ...
        with pytest.raises(RuntimeError, match="1/2") as raised:
            send_telegram_notifications_sync(self._settings(), text="alert")
