...
def test_invalid_max_history():
    ...
    with pytest.raises(ValueError, match="alert_repeat_interval"):
        MemoryMonitor(alert_repeat_interval=0)
...
def test_repeat_interval_uses_monotonic_during_wall_clock_rollback(monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.utils.memory_monitor.time.monotonic",
        lambda: next(monotonic_timestamps),
    )
    monitor = MemoryMonitor(
        threshold_mb=0.000001,
        gc_enabled=False,
