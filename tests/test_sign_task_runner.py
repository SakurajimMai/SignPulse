...
def runner_env(monkeypatch, tmp_path):
    ...
    def fake_schedule_alert(rule_id: str, **kwargs: Any) -> None:
        env.alerts.append({"rule_id": rule_id, **kwargs})
...
class TestConfigMissing:
    ...
    async def test_missing_config_fails_and_notifies(self, runner_env):
        ...
        # 统一告警已调度、运行记录已写、清理状态收敛
        assert [item["rule_id"] for item in runner_env.alerts] == [
            "sign_task_flow_fail"
        ]
...
class TestNotificationFailureCategory:
    ...
    async def test_failure_category_computed_and_passed(self, runner_env, monkeypatch):
        ...
        assert runner_env.alerts[0]["rule_id"] == "sign_task_rate_limited"
        assert "failure_category=flood_wait" in runner_env.alerts[0]["detail"]
    ...
    async def test_existing_category_preserved(self, runner_env, monkeypatch):
        ...
        await _runner_send_notifications(state)
        assert state["failure_category"] == "ai_error"
        assert runner_env.alerts[0]["rule_id"] == "sign_task_ai_fail"
        assert "failure_category=ai_error" in runner_env.alerts[0]["detail"]
