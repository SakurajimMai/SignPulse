...
class TestAdvancedSettingsApi:
    ...
    def test_save_telegram_bot_control_settings(self, client, db_session):
        resp = client.post(
            "/api/config/settings",
            json={
                "telegram_bot_control_enabled": True,
                "telegram_bot_mini_app_url": "https://panel.example.com",
                "telegram_bot_allowed_user_ids": [12345, 67890, 12345],
            },
            headers=_auth_headers(),
        )
        assert resp.status_code == 200
        ...
        got = client.get("/api/config/settings", headers=_auth_headers()).json()
        assert got["telegram_bot_control_enabled"] is True
        assert got["telegram_bot_mini_app_url"] == "https://panel.example.com/mini-app"
        assert got["telegram_bot_allowed_user_ids"] == [12345, 67890]

    def test_mini_app_root_url_is_normalized_to_route(self, client, db_session):
        resp = client.post(
            "/api/config/settings",
            json={"telegram_bot_mini_app_url": "https://panel.example.com/"},
            headers=_auth_headers(),
        )
    ...
    def test_saving_telegram_bot_control_settings_requests_runtime_reconcile(
        ...
        with patch(
            "backend.services.telegram_bot.get_telegram_bot_runtime",
            return_value=_Runtime(),
        ):
            resp = client.post(
                "/api/config/settings",
                json={"telegram_bot_control_enabled": False},
                headers=_auth_headers(),
            )
    ...
    @pytest.mark.parametrize(
        "payload",
        [
            {"telegram_bot_mini_app_url": "http://panel.example.com/mini-app"},
            {"telegram_bot_mini_app_url": "https://user:pass@panel.example.com/mini-app"},
            {"telegram_bot_allowed_user_ids": [0]},
            {"telegram_bot_allowed_user_ids": list(range(1, 22))},
        ],
    )
    def test_rejects_unsafe_telegram_bot_control_settings(
        self, client, db_session, payload
    ):
        resp = client.post(
            "/api/config/settings",
            json=payload,
    ...
    def test_telegram_bot_token_is_encrypted_at_rest(
        self, client, db_session
    ):
        token = "123456:bot-secret-value"
        resp = client.post(
            "/api/config/settings",
            json={
        ...
        settings_file = get_config_service()._get_global_settings_file()
        raw = settings_file.read_text(encoding="utf-8")
        assert token not in raw
        assert "bot-secret-value" not in raw
        got = client.get("/api/config/settings", headers=_auth_headers()).json()
        assert got["telegram_bot_token"] is None
        assert got["telegram_bot_token_set"] is True

    def test_reveal_decrypts_supported_global_secrets(self, client, db_session):
        from tg_signer.security import encrypt_secret

        plaintext = {
            "proxy_password": "test-proxy-password",
            "webdav_password": "test-webdav-password",
            "telegram_bot_token": "123456:test-bot-token",
            "smtp_password": "test-smtp-password",
        }
        persisted = {
            key: encrypt_secret(value) for key, value in plaintext.items()
        }
        persisted["global_proxy"] = (
            "socks5://test-user:test-proxy-password@proxy.example.com:1080"
        )

        with patch("backend.api.routes.config.get_config_service") as service:
            service.return_value.get_global_settings.return_value = persisted
            masked = client.get(
                "/api/config/settings", headers=_auth_headers()
            )
            revealed = client.get(
                "/api/config/settings?reveal=1", headers=_auth_headers()
            )

        assert masked.status_code == 200
        for key, value in plaintext.items():
            assert masked.json()[key] is None
            assert value not in masked.text
        assert "fernet:" not in masked.text
        assert masked.json()["global_proxy"] is None

        assert revealed.status_code == 200
        for key, value in plaintext.items():
            assert revealed.json()[key] == value
        assert "fernet:" not in revealed.text
        assert revealed.json()["global_proxy"] is None

    def test_reveal_accepts_legacy_plaintext_and_empty_secrets(
        self, client, db_session
    ):
        persisted = {
            "proxy_password": "legacy-proxy-password",
            "webdav_password": "",
        ...
        with patch("backend.api.routes.config.get_config_service") as service:
            service.return_value.get_global_settings.return_value = persisted
            response = client.get(
                "/api/config/settings?reveal=true", headers=_auth_headers()
            )

        assert response.status_code == 200
        body = response.json()
        assert body["proxy_password"] == "legacy-proxy-password"
        assert body["smtp_password"] == "legacy-smtp-password"
        assert body["webdav_password"] is None
        assert body["telegram_bot_token"] is None
        assert body["proxy_password_set"] is True
        assert body["smtp_password_set"] is True
        assert body["webdav_password_set"] is False
        assert body["telegram_bot_token_set"] is False


class TestTelegramBotRuntimeStatusApi:
    def test_runtime_status_requires_admin_session(self, client, db_session):
        resp = client.get("/api/config/settings/telegram-bot/runtime")
        assert resp.status_code == 401

    def test_runtime_status_is_token_free_and_exposes_conflict(
        self, client, db_session
    ):
        class _Runtime:
            def status(self):
        ...
        with patch(
            ...
            resp = client.get(
                "/api/config/settings/telegram-bot/runtime",
                headers=_auth_headers(),
            )

        assert resp.status_code == 200
        body = resp.json()
        assert body["state"] == "polling_conflict"
        assert body["polling_conflict"] is True
        assert body["last_error"] == "another_get_updates_consumer"
        assert body["bot"]["username"] == "ops_bot"
...
class TestWebdavBackupChain:
    ...
    def test_bot_token_masked_on_get_and_empty_keeps(self, client, db_session):
        ...
        assert decrypt_secret(stored) == unique_token
