...
def _proxy_fingerprint(proxy: dict | None) -> tuple | None:
    ...
    try:
        ...
        port = None
    return (
        str(proxy.get("scheme") or "").casefold(),
        str(proxy.get("hostname") or "").casefold(),
        port,
...
def get_client(
    ...
    **kwargs,
    ...
    if key in _CLIENT_INSTANCES:
        ...
        requested_proxy = _proxy_fingerprint(proxy)
        existing_proxy = getattr(existing, "_tg_signpulse_proxy_fingerprint", None)
        if existing_proxy is None:
            existing_proxy = _proxy_fingerprint(getattr(existing, "proxy", None))
        ...
        if (
            ...
            or requested_proxy != existing_proxy
    ...
    client._tg_signpulse_proxy_fingerprint = _proxy_fingerprint(proxy)
