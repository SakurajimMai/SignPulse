...
RUN apt-get update && apt-get install -y --no-install-recommends build-essential tzdata gosu p7zip-full curl ca-certificates && \
...
# Apate 用于把带密码的 7z 伪装成 MP4 再上传百度；失败不阻断镜像构建。
ARG TARGETARCH
RUN arch="${TARGETARCH:-amd64}"; \
  rust_arch="x86_64"; \
  if [ "$arch" = "arm64" ] || [ "$arch" = "aarch64" ]; then rust_arch="aarch64"; fi; \
  if [ "$rust_arch" = "aarch64" ]; then \
    url="https://github.com/SakurajimMai/apate/releases/download/latest/apate-main-linux-aarch64.tar.gz"; \
  else \
    url="https://github.com/SakurajimMai/apate/releases/download/latest/apate-main-linux-x86_64.tar.gz"; \
  fi; \
  if curl -fsSL "$url" -o /tmp/apate.tgz; then \
    mkdir -p /tmp/apate-extract; \
    tar -xzf /tmp/apate.tgz -C /tmp/apate-extract || true; \
    bin="$(find /tmp/apate-extract /usr/local/bin -maxdepth 3 -type f -name apate 2>/dev/null | head -n 1)"; \
    if [ -n "$bin" ]; then cp "$bin" /usr/local/bin/apate && chmod +x /usr/local/bin/apate; fi; \
    rm -rf /tmp/apate.tgz /tmp/apate-extract; \
  else \
    echo "optional apate binary not downloaded"; \
  fi
